...
sbit CE   = P2^3;     //RX/TX模式选择端
sbit CSN  = P2^0;    //SPI片选端//就是SS
sbit SCLK = P2^4;	 //SPI时钟端
sbit MOSI = P2^1;	 //SPI主机输出从机输入端
sbit MISO = P2^5;	 //SPI主机输出从机输出端
sbit IRQ  = P2^2;   //可屏蔽中断端		
/*
sbit CE   = P2^5;     //RX/TX模式选择端
sbit CSN  = P2^0;    //SPI片选端//就是SS
sbit SCLK = P2^4;	 //SPI时钟端
sbit MOSI = P2^1;	 //SPI主机输出从机输入端
...
sbit IRQ  = P2^2;   //可屏蔽中断端
*/	
uchar RevTempDate[4]={0};//最后一位用来存放结束标志
